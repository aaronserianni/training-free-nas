...
"""The modified ELECTRA encoder, modified to suit the FlexiBERT search space
"""
...
class BertConfig(object):
  def __init__(self,
               ...
               is_weighted_multiplcative=False,
               is_dft=False,
               is_dct=False,
               is_dsc=False,
               conv_kernel_size=5):
    ...
    self.is_weighted_multiplcative = is_weighted_multiplcative
    self.is_dft = is_dft
    self.is_dct = is_dct
    self.is_dsc=is_dsc
    ...
    with tf.variable_scope(scope, default_name="electra"):
      ...
      with tf.variable_scope("encoder"):
        ...
        (self.all_layer_outputs, self.attn_maps) = transformer_model(
            ...
            nas_config=bert_config.nas_config,
            attention_mask=attention_mask,
            hidden_size=bert_config.hidden_size,
            num_hidden_layers=bert_config.num_hidden_layers,
            num_attention_heads=bert_config.num_attention_heads,
            num_intermediate=bert_config.num_intermediate,
            ...
            is_weighted_multiplcative=bert_config.is_weighted_multiplcative,
            is_dft=bert_config.is_dft,
            is_dct=bert_config.is_dct,
            is_dsc=bert_config.is_dsc,
...
def attention_layer(from_tensor,
                    ...
                    to_seq_length=None,
                    is_weighted_multiplicative=False,
                    is_dft=False,
                    is_dct=False,
                    is_dsc=False,
                    conv_kernel_size=5,
  ...
  if operation_type is "SA":
    is_dft, is_dct, is_dsc, is_weighted_multiplicative = False
    if operation_type is "SDP":
      is_weighted_multiplicative=False
    if operation_type is "WMA":
      is_weighted_multiplicative=False
  if operation_type is "LT":
    is_dft, is_dct, is_dsc, is_weighted_multiplicative = False
    if operation_parameter is "DFT":
      is_dft = True
    if operation_parameter is "DCT":
      is_dct = True
  if operation_type is "DSC":
    is_dft, is_dct, is_weighted_multiplicative = False
    is_dsc = True
    if isinstance(operation_parameter, int):
      conv_kernel_size = operation_parameter
  ...
  if is_dft:
    ...
    # `input_layer` = [B, N, F, H]
    input_layer = transpose_for_scores(input_layer, batch_size,
                                       num_attention_heads, from_seq_length,
                                       size_per_head)
    # `fourier_layer` = [B, N, F, H]
    fourier_score = tf.math.real(tf.keras.layers.Lambda(lambda v: tf.signal.fft(tf.cast(v,tf.complex64)))(input_layer))

    # 'fourier_layer' = [B, N, H, F]
    fourier_score = tf.transpose(fourier_score, [0, 1, 3, 2])

    # `fourier_layer` = [B, N, H, F]
    fourier_score = tf.math.real(tf.keras.layers.Lambda(lambda v: tf.signal.fft(tf.cast(v,tf.complex64)))(input_layer))

    # `context_layer` = [B, F, N, H]
    context_layer = tf.transpose(fourier_score, [0, 3, 1, 2])
    attention_probs = fourier_score
  elif is_dct:
    # `input_layer` = [B*F, N*H]
    input_layer = tf.layers.dense(
        from_tensor_2d,
    ...
    input_layer = transpose_for_scores(input_layer, batch_size,
                                       ...
                                       size_per_head)
    # `fourier_layer` = [B, N, F, H]
    cosine_score = tf.math.real(tf.keras.layers.Lambda(lambda v: tf.signal.dct(v))(input_layer))

    # 'fourier_layer' = [B, N, H, F]
    cosine_score = tf.transpose(cosine_score, [0, 1, 3, 2])

    # `fourier_layer` = [B, N, H, F]
    cosine_score = tf.math.real(tf.keras.layers.Lambda(lambda v: tf.signal.dct(v))(input_layer))

    # `context_layer` = [B, F, N, H]
    context_layer = tf.transpose(cosine_score, [0, 3, 1, 2])
    attention_probs = cosine_score
  elif is_dsc:
     # `query_layer` = [B*F, N*H]
    ...
    # [B,T, N*H]
    key_conv_attn_layer = reshape_for_conv(to_tensor_2d, batch_size, num_attention_heads,
    ...
    attention_probs = conv_out_layer
  else:
    # `query_layer` = [B*F, N*H]
    query_layer = tf.layers.dense(
        from_tensor_2d,
    ...
    if is_weighted_multiplicative:
      # `weighted_query_layer` = [B*F, N*H]
      query_layer = tf.layers.dense(
        query_layer,
        num_attention_heads * size_per_head,
        use_bias=False,
...
def transformer_model(input_tensor,
  ...
  if nas_config is not None:
    for layer_idx, encoder_layer in enumerate(nas_config['encoder_layers']):
      with tf.variable_scope("layer_%d" % layer_idx):
        with tf.variable_scope("attention"):
          if hidden_size % encoder_layer['num_operation_heads'] != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, encoder_layer['num_operation_heads']))
          attention_head_size = int(hidden_size / encoder_layer['num_operation_heads'])
          ...
          with tf.variable_scope("self"):
            attention_head, probs = attention_layer(
                ...
                attention_mask=attention_mask,
                num_attention_heads=encoder_layer['num_operation_heads'],
                ...
                is_weighted_multiplicative=is_weighted_multiplcative,
                is_dft=is_dft,
                is_dct=is_dct,
                is_dsc=is_dsc,
                conv_kernel_size=conv_kernel_size,
                operation_type=encoder_layer['operation_type'],
                operation_parameter=encoder_layer['operation_parameter'])
        ...
        with tf.variable_scope("intermediate"):
          intermediate_output = tf.layers.dense(
              ...
              kernel_initializer=create_initializer(initializer_range))
          for interediate_idx in range(encoder_layer['num_feed_forward']-1):
            with tf.variable_scope("intermediate_layer_%d" % (interediate_idx+1)):
              prev_intermediate_output = intermediate_output
              intermediate_output = tf.layers.dense(
                  prev_intermediate_output,
                  encoder_layer['feed_forward_dimension'],
                  activation=intermediate_act_fn,
                  kernel_initializer=create_initializer(initializer_range))
      ...
      with tf.variable_scope("layer_%d" % layer_idx):
        ...
        with tf.variable_scope("intermediate"):
          intermediate_output = tf.layers.dense(
              ...
              activation=intermediate_act_fn,
              kernel_initializer=create_initializer(initializer_range))
          for interediate_idx in range(num_intermediate-1):
            with tf.variable_scope("intermediate_layer_%d" % (interediate_idx+1)):
              prev_intermediate_output = intermediate_output
              intermediate_output = tf.layers.dense(
                  prev_intermediate_output,
